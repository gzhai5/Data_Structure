...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    unsigned long hash_hash = hashes::hash(key, size);
    std::pair<K, V>* pair_pair = new std::pair<K, V>(key, value);
    unsigned long step = hashes::secondary_hash(key, size);
    while (table[hash_hash] != NULL){
        if (hash_hash < step){
            hash_hash = hash_hash + size - step;
        }
        else{
            hash_hash = hash_hash - step;
        }
    }
