...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (!singleQueue.empty() || mergeQueue.size() != 1){
        TreeNode * remove1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode * remove2 = removeSmallest(singleQueue, mergeQueue);
        int internal_freq = remove1 -> freq.getFrequency() + remove2 -> freq.getFrequency();
        TreeNode * internal = new TreeNode(internal_freq);
        mergeQueue.push(internal);
        internal -> left = remove1;
        internal -> right = remove2;
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (!bfile.hasBits()){
        ...
        TreeNode * leaf = new TreeNode(leafFreq);
        return leaf;
    }
    else{
