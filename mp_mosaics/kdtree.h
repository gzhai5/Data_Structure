...
class KDTree
{
  ...
  private:
    ...
    void buildTree(vector<Point<Dim>>& points, int dimension, KDTreeNode *& subroot, int start, int end) {
      if (start > end){     //base case
        return;
      }
      int median = (start + end) / 2;
      qucikselect(points, median, dimension, start, end);
      subroot = new KDTreeNode(points[median]);
      buildTree(points, (dimension + 1) % Dim, subroot -> left, start, median - 1);
      buildTree(points, (dimension + 1) % Dim, subroot -> right, median + 1, end);
    }




    void qucikselect(vector<Point<Dim>>& points, int median, int dim, int start, int end) {
      if (start >= end){
        return;
      }
      Point<Dim> x = points[end];
      int key = median;
      int index = start;
      for (int i = start; i < end; i++){
        if (smallerDimVal(points[i], x, dim)){
          std::swap(points[index], points[i]);
          index++;
        }
      }
      std::swap(points[index], points[end]);
      if (index == key){
        return;
      }
      if (index > key){
        qucikselect(points, key, dim, start, index - 1);
      } 
      else{
        qucikselect(points, key - index, dim, index + 1, end);
      }
    }
    ...
    void CopyTree(const KDTreeNode *& root, KDTreeNode *& other_root) {
      if (other_root == NULL){
        return;
      }
      root = new KDTreeNode();
	    root -> point = other_root -> point;
      CopyTree(root -> left, root -> left);
      CopyTree(root -> right, root -> right);
    }
    ...
    void findNearestNeighbor(const Point<Dim>& query, Point<Dim>& currentBest, KDTreeNode* root, int d) const {
        d %= Dim;
        if (root == NULL) {
          return;
        }
        double dis_cur_que = 0;
        if (smallerDimVal(query, root -> point, d)){
          findNearestNeighbor(query, currentBest, root -> left, d + 1);
          if (shouldReplace(query, currentBest, root -> point)){
            currentBest = root->point;
          }  
          int count = square((root -> point[d] - query[d]));
          for (int i = 0; i < Dim; i++){
            dis_cur_que += square((currentBest[i] - query[i]));
          }
          if (dis_cur_que >= count){
            if (root->right != NULL){
              findNearestNeighbor(query, currentBest, root -> right, (d + 1) % Dim);
            }
          }
        } 
        else{
          findNearestNeighbor(query, currentBest, root -> right, d + 1);
          if (shouldReplace(query, currentBest, root -> point)){
            currentBest = root -> point;
          }
          int count = square((root -> point[d] - query[d]));
          ...
          if (dis_cur_que >= count){
            if (root->left != NULL){
              findNearestNeighbor(query, currentBest, root->left, (d + 1) % Dim);
            }
          }
        }
    }
